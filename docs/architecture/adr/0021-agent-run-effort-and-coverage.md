...
The adapter attributes only existing bounded `progressSummary` and `usage` fields when a persisted workflow summary passes the supported v1 envelope checks: known envelope/child fields, bounded producer IDs, `inventoryComplete: true`, a recognized workflow state, and bounded arrays. `mode` must be `"workflow"` and `workflowRunId` must exactly equal `details.runId`; `parentToolCallId` is validated but never compared with the current tool-call ID because async publication can differ. A bounded `workflowKey` must exactly match one `childId`; missing, mismatched, or duplicate keys withhold only that key, while unrelated exact keys remain eligible. An unsupported envelope or child row rejects that publication’s correlation. The adapter attributes only result `progressSummary` and `usage` to existing workflow child rows; it does not infer effort from summary rows. It does not change row identity, status, parentage, or ordering, and does not merge result and workflow publication rows. No status/success inference or additive child accounting is introduced. The 0.70.1 evidence remains historical and unchanged.
