...
The interactive application uses authored sources and generated package assets:

```text
scripts/web/                 # authored browser sources, never shipped
  route.js
  range.js
  client.js
src/ui/web/                  # shipped package assets
  shell.html
  style.css
  client.bundle.js           # deterministic esbuild output
```

`npm run build:web` concatenates the three authored sources in order and bundles
them into one deterministic classic asset; `npm run build:web:check` compares the
generated bytes, and runs from `prepack` and the pre-commit hook, so a stale
asset cannot ship. Generated output is named `.bundle.js` so a diff never has to
guess which file is authored. The build does not transform behaviour: esbuild
joins and minifies, it does not rewrite the browser logic.

The server serves three assets: `/`, `/style.css`, and `/client.js` (the
generated bundle, under its stable external path). The static snapshot does not
include the client; it may inline the known stylesheet. The bundle installs one
...
Pre-M8.6 adopted build-time bundling after comparing the unbundled client with a
bundled candidate. It changes asset assembly only: no runtime TypeScript
transformation, module loader, CDN, or network resource is introduced, and server
