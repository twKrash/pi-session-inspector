...
Exactly one dated projection exists: `sessionDatedUsage(session)` in `src/ui/dated-usage.ts` reads the builder's `usage.lines` and joins per-date model rows to their line by `ownerId`. Every production consumer — the current views, history/global, and the folded chart rows — consumes those rows. The report is used to **assert** reconciliation (`sum(usageByDate) === SessionReport.usage`), never to produce the dates, and no second walk of `SessionReport` timestamps is permitted. The one documented exception is the legacy single-section `renderHtml(HtmlReport)` adapter (no production caller; deferred), whose current-view daily rows keep the pre-attribution `buildDailyActivityRows` bucketing because it has no canonical session to project from. Tools, agents, and errors are filtered from their own canonical rows (one row per call/run/error); date-indexed duplicates for them are explicitly rejected.
