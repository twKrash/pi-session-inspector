...
- Wording is part of the contract, not styling: partial aggregates read `Known native cost` / `Known tokens`; agent and tool breakdowns read `Known child …` / `Known tokens` with their `n of m` fraction; unavailable never renders as `$0.00` or `0`. The bare word `Total` is never used for a partial sum (the JSON field name `usage.total` is kept for compatibility).
...
Exactly one dated projection exists: `sessionDatedUsage(session)` in `src/ui/dated-usage.ts` reads the builder's `usage.lines` and joins per-date model rows to their line by `ownerId`. Every consumer — the current views, history/global, and the folded chart rows — consumes those rows. The report is used to **assert** reconciliation (`sum(usageByDate) === SessionReport.usage`), never to produce the dates, and no second walk of `SessionReport` timestamps is permitted. Tools, agents, and errors are filtered from their own canonical rows (one row per call/run/error); date-indexed duplicates for them are explicitly rejected.
