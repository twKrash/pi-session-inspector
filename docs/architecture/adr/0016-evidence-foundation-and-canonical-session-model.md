...
- every other field on ADR 0014's "never read" list — `origin`, `value`, `matchedPattern`, `agentName`, `forwarding` — remains prohibited; only the permission adapter may read the raw request ID locally to derive the authorized opaque attribution.
