...
**Boundary on read.** A tracked session whose evidence has never been folded has no boundary, and R50 publishes no counter total without one — so a session whose detached `session_start` pass no-oped reported its durable WAL evidence as `unavailable` on every read, including a policy-denied command whose producer had published `permissions:decision`. A read that finds no checkpoint therefore runs that same maintenance pass once for that session and root (bounded attempt memo, at most one attempt per session per process, the oldest attempt forgotten at the cap), then re-reads. The pass is the existing one — same lease, same idempotent merge, same writer id discipline — and stays observer-only: Inspector's own derived state is written, Pi's session file never is, and every failure leaves the read exactly as it was. This is the only write a read may perform, it only ever creates a boundary that was absent, and it never changes a boundary that exists.
