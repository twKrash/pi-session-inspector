...
**Fold and cursor discipline.** Recovery folds only envelopes strictly after each writer's checkpoint WAL cursor into `deltaCounters`. Checkpoint aggregates (`aggregates.integrationCounters`, `aggregates.skillInvocations`, `aggregates.skillOverflowInvocations`, `aggregates.presence`) mean "already folded". Reports read `effective = merge(checkpointAggregates, deltaCounters)`; maintenance persists that same merge exactly once per pass. Repeated folds, repeated reads, and `/resume` writer shards therefore never double-count, and a report read never rewrites folded state. Fold sums are integer and order-independent, so JSON output stays byte-stable (PRD-01).

**Runtime ownership.** Live registrations belong to the session runtime that created them, not to the process. Pi replaces a session runtime — emitting `session_shutdown` for the outgoing extension instance and removing its listeners — while the Inspector module itself stays cached, so a registration kept beyond its runtime would be inert: the resumed session would hold listeners Pi has already dropped and silently report no permission or skill telemetry. One live registration exists per active session runtime; the runtime disposes its own registration when it ends (every `session_shutdown` reason), and a registration owned by a replaced runtime is disposed rather than reused, so a resumed session subscribes afresh. Ownership is keyed by the Inspector root plus the session id, and the runtime identity is part of the key.

**Boundary on read.** A tracked session whose evidence has never been folded has no boundary, and R50 publishes no counter total without one — so a session whose detached `session_start` pass no-oped reported its durable WAL evidence as `unavailable` on every read, including a policy-denied command whose producer had published `permissions:decision`. A read that finds no checkpoint therefore runs that same maintenance pass once for that session, root, and runtime (bounded attempt memo: one attempt per session, root, and runtime, with the oldest forgotten once 64 are remembered, so a session re-attempts only after 64 others), then re-reads. The memo is runtime-scoped for the same reason as registration ownership: a session that spent its one attempt before Pi replaced its runtime gets a fresh attempt once it is resumed, while a healthy runtime still folds at most once and never re-runs maintenance on every read. A read served by a composition that outlives the replacement (the loopback UI server keeps the session and runtime it was started with) keeps that runtime's own attempt, because it also keeps reading that same session. The pass is the existing one — same lease, same idempotent merge, same writer id discipline — and stays observer-only: Inspector's own derived state is written, Pi's session file never is, and every failure leaves the read exactly as it was. This is the only write a read may perform, it only ever creates a boundary that was absent, and it never changes a boundary that exists.
