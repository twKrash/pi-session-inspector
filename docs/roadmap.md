...
**Delivered (build seam).** `esbuild` bundles the readable `scripts/web/`
sources (route, range, client) into one deterministic classic asset,
`src/ui/web/client.bundle.js`, which the server serves as `/client.js`;
`/route.js` and `/range.js` are no longer runtime assets.
`npm run build:web:check` compares the generated bytes and runs from `prepack`
and the pre-commit hook. Browser behavior, the API, and the accepted server
architecture are unchanged.

**Delivered (localization boundary).** `src/ui/i18n/catalog.ts` is the one
English catalog and `src/ui/i18n.ts` the Inspector-owned synchronous translator;
the TypeScript renderers, the snapshot, and the browser bundle resolve copy
through it, so the browser's duplicate `COPY` table and its custom interpolation
helpers are gone. Locale selection stays explicit English with local fallback —
no detector, backend, persistence, CDN, or network load. The chart
representation remains open in this milestone.
