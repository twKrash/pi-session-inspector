...
| --- | --- | --- | --- | --- |
| Terminal success/failure/interruption | partial | A: declared `exitCode`, `processSignal`, `outputState` on `results[]`; A: child `success` on nested completion results; B: dynamic `state`/`success` where retained on foreground results and outer completions | session-scoped opaque digest of `runId`; bounded agent label only | execution completion, not semantic task success; dynamic foreground state/success are not declared `SingleResult` fields; cancellation is unavailable |
...
| `durationMs` | unavailable | Producer construction A: `progress.durationMs`; terminal `progressSummary.durationMs`; no proven projection into declared `SingleResult`/`WaitCompletion` publication or persisted `details` fixture surface | no timestamp or live-state joins | `observedAt`, entry timestamps, and live status files do not establish historical duration; field remains absent from fixture |
| `toolCount` | unavailable | Producer construction A: `progress.toolCount`; terminal `progressSummary.toolCount`; no proven projection into declared publication arrays or persisted `details` | never derived from `toolCalls[].length` | field remains absent from fixture; no cross-surface inference |
| `turnCount` | unavailable | Producer construction A: `progress.turnCount = result.usage.turns`; terminal `progressSummary.turnCount`; no proven projection into declared publication arrays or persisted `details` | bounded producer construction only | not equated with `usage.turns` or native generations; field remains absent from fixture |
...
The fixture uses these bounded status/error fields: `state`, `success`, `exitCode`, `processSignal`, and `outputState`. `exitCode`, `processSignal`, and `outputState` are declared on `SingleResult`; `success` and `outputState` are declared on `WaitCompletionChild`; `state` and `archivePath` are declared on `WaitCompletion`; `Details.results` and `Details.completions` are declared publication arrays; and `WorkflowChildSummary.children` declares child identity/state but no usage. `state` and `success` on foreground `results[]`, and `success` on the outer completion, are dynamic fields accepted by Inspector for sanitized evidence only, not claims made by the pinned declarations. No `ArchiveReference` declaration is asserted: `archivePath` is `WaitCompletion.archivePath` (line 1235), reduced to the bounded available/missing verdict.

| Public declaration symbol | Exact field path | Persisted publication surface | Declaration / dynamic distinction | 13B coverage |
| --- | --- | --- | --- | --- |
| `SingleResult` | `details.results[].index`, `.runId`, `.agent`, `.exitCode`, `.processSignal`, `.usage`, `.outputState` | `subagent` tool-result `details.results[]` | fields listed are declared; fixture-only `state`/`success` are dynamic Inspector-accepted fields | supported for identity/usage when complete; effort fields absent in this fixture are unavailable |
| `WaitCompletion` | `details.completions[].runId`, `.state`, `.archivePath`, `.results` | `subagent_wait` tool-result `details.completions[]` | `state` and `archivePath` are declared; outer `success` is dynamic; nested child fields use `WaitCompletionChild` | supported for terminal state and replacement usage; cancellation unavailable |
...
The checked producer implementation proves construction of `progress.durationMs`, `progress.toolCount`, and `progress.turnCount = result.usage.turns`, plus terminal `progressSummary` values and snapshot carriage. It does **not** prove those values reach persisted `details.results[]`, `details.completions[]`, nested completion results, or workflow summaries. They are therefore unavailable on the persisted audit surface and are intentionally absent from the fixture. `usage.turns` is separately supported only where it is actually present in a persisted usage group; it remains usage metadata, not a native generation count. `observedAt` is only the publishing Pi entry timestamp; no duration is inferred from timestamps or live status files. Cancellation is unavailable: the fixture contains no producer `state: "cancelled"` row, and no pinned declaration evidence proves cancellation semantics. The matching fixture assistant/tool-result pair carries a bounded `details.auditOnly.cancellation` annotation with candidate `cancelled` and reason `unavailable`; this is synthetic audit metadata, not a `pi-subagents` field, and the existing evidence parser intentionally ignores it.
