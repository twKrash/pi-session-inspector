...
The checked producer implementation proves live construction of `progress.durationMs`, `progress.toolCount`, and `progress.turnCount = result.usage.turns`. In the pinned single foreground final path, `compactForegroundResult` retains `progressSummary`, so terminal `progressSummary.durationMs` and `progressSummary.toolCount` persist in `subagent` tool-result `details.results[]`; this supports those fields for foreground terminal, failure, and interruption results. Final compaction strips `progress`, and `ProgressSummary` has no `turnCount`, so `turnCount` remains unavailable on stable final publication. No equivalent persistence is proven for `subagent_wait` `details.completions[]`, nested completion results, or workflow summaries; overall effort coverage is therefore partial. `usage.turns` is separately supported only where present in a persisted usage group; it is usage metadata, not native generations or `turnCount`. `observedAt` is only the publishing Pi entry timestamp; no duration is inferred from timestamps or live status files. Cancellation is unavailable: the fixture contains no producer `state: "cancelled"` row, and no pinned declaration evidence proves cancellation semantics. The matching fixture assistant/tool-result pair carries a bounded `details.auditOnly.cancellation` annotation with candidate `cancelled` and reason `unavailable`; this is synthetic audit metadata, not a `pi-subagents` field, and the existing evidence parser intentionally ignores it.
