...
| usage.turns | supported | persisted usage groups in foreground `details.results[].usage.turns`, `details.completions[].usage.turns`, nested completion `results[].usage.turns`, and explicitly supplied workflow dynamic paths | Lane A; usage metadata only, not `turnCount` or native generations |
...
| Usage | partial | `details.results[].usage.*`; `details.completions[].usage.*`; nested completion `results[].usage.*`; workflow dynamic child usage path when explicitly supplied | Lane A; supported-on-foreground and partial across surfaces; child breakdown is non-additive and malformed groups remain unavailable |
| Cost | partial | `details.results[].usage.cost`; `details.completions[].usage.cost`; nested completion `results[].usage.cost`; workflow dynamic child `usage.cost` when explicitly supplied | Lane A; supported-on-foreground and partial across surfaces; never synthesize from time or missing tokens |
