...
| usage.turns | supported | persisted usage groups in foreground `details.results[].usage.turns`, `details.completions[].usage.turns`, and nested completion `results[].usage.turns` | Lane A for declared usage metadata; explicitly supplied workflow dynamic child usage is Lane B and partial, not a generation count |
...
| Usage | partial | `details.results[].usage.*`; `details.completions[].usage.*`; nested completion `results[].usage.*`; workflow dynamic child usage path when explicitly supplied | Lane A for declared foreground/completion usage; workflow dynamic child usage is Lane B and partial; supported-on-foreground and partial across surfaces; child breakdown is non-additive and malformed groups remain unavailable |
| Cost | partial | `details.results[].usage.cost`; `details.completions[].usage.cost`; nested completion `results[].usage.cost`; workflow dynamic child `usage.cost` when explicitly supplied | Lane A for declared foreground/completion usage; workflow dynamic child usage is Lane B and partial; supported-on-foreground and partial across surfaces; never synthesize from time or missing tokens |
