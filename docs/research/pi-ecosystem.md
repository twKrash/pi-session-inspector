...
| --- | --- | --- | --- | --- |
| `SingleResult` | `details.runId`; `details.results[].index`, `.agent`, `.exitCode`, `.processSignal`, `.usage`, `.outputState` | foreground `subagent` tool-result `details` and its `results[]` | `details.runId` is the foreground run identity; the listed `results[]` fields are declared, while fixture-only `state`/`success` are dynamic Inspector-accepted fields. Completion and nested run IDs belong to their own `WaitCompletion`/`WaitCompletionChild` surfaces. | supported for identity/usage when complete; effort fields are available/proven on final foreground `results[]` when `progressSummary` is present; `turnCount` remains unavailable and effort is partial/unavailable on other publication surfaces |
...
The checked producer implementation proves live construction of `progress.durationMs`, `progress.toolCount`, and `progress.turnCount = result.usage.turns`. In the pinned single foreground final path, `compactForegroundResult` retains `progressSummary`, so terminal `progressSummary.durationMs` and `progressSummary.toolCount` persist in `subagent` tool-result `details.results[]`; effort fields are therefore available/proven on final foreground `results[]` when `progressSummary` is present, including terminal, failure, and interruption results. `turnCount` remains unavailable because final compaction strips `progress` and `ProgressSummary` has no `turnCount`; effort remains partial or unavailable on `subagent_wait` completions, nested completion results, and workflow summaries. `usage.turns` is separately supported only where present in a persisted usage group; it is usage metadata, not native generations or `turnCount`. `observedAt` is only the publishing Pi entry timestamp; no duration is inferred from timestamps or live status files. Cancellation is unavailable: the fixture contains no producer `state: "cancelled"` row, and no pinned declaration evidence proves cancellation semantics. The matching fixture assistant/tool-result pair carries a bounded `details.auditOnly.cancellation` annotation with candidate `cancelled` and reason `unavailable`; this is synthetic audit metadata, not a `pi-subagents` field, and the existing evidence parser intentionally ignores it.
