...
| [`subagents`](https://github.com/nicobailon/pi-subagents) | native `subagent`, `subagent_wait`, `bg_wait`, `subagent_supervisor` tools | persisted tool results (`details.results[]` / `details.completions[]`), plus current-session-only referenced `status.json` v3 and the per-child `_meta.json` referenced by a detached foreground result | no | yes: runs and native subagent activity reach the Agents view | The integration row declares **no counters**; its schema is intentionally empty. A run's status, effort and usage coverage, async kind, and detached disposition come from persisted rows; a referenced lifecycle artifact (ADR 0022) or the referenced per-child metadata artifact (ADR 0023) may only fill an already-proven current-session row and never creates one |
