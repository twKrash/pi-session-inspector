...
- Refresh triggers, all bounded and idempotent: (1) session start after tracking promotion; (2) a `resources_discover` event with `reason === "reload"` (Pi's own resource-reload signal, so newly loaded skills/prompts/extensions are picked up); (3) any report load, where Inspector re-reads `getCommands()` + `getAllTools()`, hashes the just-observed sanitized row set, and compares it with the persisted snapshot. Every refresh hash-compares the just-observed sanitized row set with the persisted snapshot — a changed hash triggers one atomic rewrite, an unchanged hash writes nothing. A report load keeps the in-memory observed snapshot (`presence`, the live-counter skill allowlist, and report injection) in sync with the refreshed one, so dynamic runtime registrations (for example an extension registering agents/commands after start, or an MCP server connecting late) are captured at the next report load at the latest. A producer read that is missing or throws keeps the last readable snapshot and never fabricates an empty inventory. Inspector never polls and never watches the filesystem.
