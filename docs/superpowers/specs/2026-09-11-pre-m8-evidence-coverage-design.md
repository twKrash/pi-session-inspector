...
- output: `counters: Record<IntegrationKey, Record<string, number>>`, `skillInvocations: Record<string, number>` (inventory-validated skill name → count), `otherInvocations` (exact invocation count whose key exceeded the 64-key cap), and `presence: { permission: boolean }`, plus the sealed-cursor information already available. Skill names are **not** `IntegrationKey`s, so they get their own bounded map rather than being forced into the integration counter shape or the inventory-count field.
- **Checkpoint/delta discipline.** Recovery returns `deltaCounters` — only telemetry **strictly after** each writer's checkpoint WAL cursor. Checkpoint aggregates mean *already folded*. Reports use `effective = merge(checkpointAggregates, deltaCounters)`; maintenance persists `merge(checkpointAggregates, deltaCounters)` exactly once per pass. Repeated passes and repeated reads therefore never double-count, and a report read never writes a checkpoint.
- **Durable presence.** `permission.ready` folds into `presence.permission` (boolean) and never into a counter key. It survives resume/history so a session whose permission bus was observed in an earlier process still reports `present` (or stays `unknown`, never `absent`).
...
- Additive optional fields (all validated, `schemaVersion` stays `1`; absence means "not folded yet"):
  ...
  - `aggregates.skillInvocations` — `Record<sanitizedSkillName, number>`: keys must match the §4.4 skill-name grammar (≤64 bytes), values are safe non-negative integers, capped at `≤64` entries;
  - `aggregates.skillOverflowInvocations` — safe non-negative integer: the **exact invocation count** for names beyond the 64-key cap. Overflow is counted per invocation (not per omitted key) so incremental folds, resume, and checkpoint merges stay exact;
  - `aggregates.presence` — `{ permission?: boolean }` only: durable presence metadata, validated as a boolean, never an activity counter;
...
- Skills: `items` is the union of inventory names and folded/aggregated invocation names; `explicitInvocations` comes from WAL fold + `aggregates.skillInvocations`; `invocationCount = sum(counts) + otherInvocations` (exact, and `otherInvocations` is surfaced separately so the UI can render "+ N other invocations"); `invocationState` is `supported` when any count exists, else `unavailable`; `state` reports inventory availability only, so an expired inventory shows counted names with `state: "unavailable"`, never a fabricated list.
- Presence: the `permission` row is `present` when the live bus signal **or** the persisted `presence.permission` aggregate is set; other rows come from the session's inventory snapshot signals where a snapshot exists, else `unknown`.
...
- Refresh triggers, all bounded and idempotent: (1) session start after tracking promotion; (2) a `resources_discover` event with `reason === "reload"` (Pi's own resource-reload signal, so newly loaded skills/prompts/extensions are picked up); (3) any current-session report load (`ui`, `tui current`/`ledger`, `json current`), where Inspector re-reads `getCommands()` + `getAllTools()`, hashes the just-observed sanitized row set, and compares it with the persisted snapshot; history/global exports do not re-read the producer. Every refresh hash-compares the just-observed sanitized row set with the persisted snapshot — a changed hash triggers one atomic rewrite, an unchanged hash writes nothing. A current-session report load keeps the in-memory observed snapshot (`presence`, the live-counter skill allowlist, and report injection) in sync with the refreshed one, so dynamic runtime registrations (for example an extension registering agents/commands after start, or an MCP server connecting late) are captured at the next current-session report load at the latest. A producer read that is missing or throws keeps the last readable snapshot and never fabricates an empty inventory. Inspector never polls and never watches the filesystem.
...
type SessionReport = {
  ...
  skills: {
    ...
    otherInvocations: number | null;   // exact count for names beyond the 64-key cap
  };
...
| --- | --- | --- |
| `aggregates.integrationCounters`, `aggregates.skillInvocations`, `aggregates.skillOverflowInvocations`, `aggregates.presence`, `aggregates.resourceCounts` in checkpoint | additive, validated, `schemaVersion` stays 1 | absence = not folded; skill map capped at 64 validated names; `presence` accepts only the boolean `permission` key; strict validation retained |
...
6. **Expiry and aggregate retention** — sealed + pruned detail with folded counters reports counters, not zero; sealed without fold reports `unavailable`; after `inventory.json` deletion the Skills tab still shows exact per-skill invocation counts (from `aggregates.skillInvocations`) with inventory `state: "unavailable"`; `aggregates.skillInvocations` validation rejects unsafe values and oversize/non-grammar names and caps at 64 entries, while `aggregates.skillOverflowInvocations` keeps the overflow exact across incremental folds; `aggregates.presence` accepts only the boolean `permission` key; `resourceCounts` reports inventory totals after expiry.
