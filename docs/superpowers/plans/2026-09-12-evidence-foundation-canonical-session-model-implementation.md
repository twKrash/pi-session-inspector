...
`src/pi/scope.ts` resolves the first record for which `isTrackingMarkerRecord` is true (`type === "custom"`, `customType === "session-inspector:tracking-start"`, `data.schemaVersion === 1`), counts every other matching record as a duplicate, then locates that marker's node in the graph and uses the node's `appendOrdinal` as the boundary (controller ruling R16). Scope is every node with a strictly greater `appendOrdinal`, so a records list that is not the same list that produced the nodes can never leak a pre-boundary node. If the marker id is missing, is not a bounded string, or has no matching node in the graph, the result is `{ state: "unavailable", reason: "tracking-marker-missing" }`. `active` additionally requires the leaf node to exist and walks `parentId` over the graph with a visited set (cycle-safe), returning root-first ancestry. Unknown semantic nodes are ordinary graph nodes here and are never filtered by type. `src/pi/sessions.ts` must keep exporting `hasTrackingStartMarker` and `selectScope`, and make `isTrackingStartMarker` delegate to `isTrackingMarkerRecord` so the rule lives in one place; do not duplicate the predicate.
...
- `src/integrations/subagents.ts`: pass the publishing result's entry timestamp and `tool:<toolCallId>` into `pushRun`, plus bounded `model`/`thinking` via `boundedProducerLabel`, and `failure` derived only from closed producer evidence, with this exact precedence (controller ruling R21, aligning the foundation with the approved downstream report plan): bounded `processSignal` → `process-signal` (detail = signal), else non-zero `exitCode` → `exit-nonzero` (detail = code), else `success === false` → `completion-failed`, else `outputState === "absent"` → `output-absent`. Within one publishing entry the canonical later-wins order is `details.results[]`, then `details.completions[]`, then `workflowChildren.children[]`, because completions carry the terminal evidence (controller ruling R22). Run identity uses `subagent-` plus `canonicalOpaqueDigest("subagent-run", sessionId, rawRunId)` from `src/core/opaque-id.ts` (controller ruling R23); the raw producer run id never leaves the adapter. Repeated run ids merge per spec §8.5: latest publication wins `observedAt`/`evidenceToolId`; conflicting non-missing `agent`/`parentId` drops the field and increments `cooperative-evidence-conflict`; terminal→running regression sets `status: "unknown"` and increments the same diagnostic; usage is selected, never summed.
...
2. `src/storage/maintenance.ts`: stamp `evidence.checkpointedAt` from `now()` **only when the materialized checkpoint content actually changes** — compare the candidate against the stored checkpoint with `checkpointedAt` removed from both, keep the stored value and skip the write when nothing else differs (controller ruling R34, which also makes plan Task 17's prune idempotence achievable). Do **not** write `evidence.usageCoverage` from maintenance: the reducer exposes no per-owner usage-presence signal, so any value here would be fabricated (controller ruling R33); the field stays optional in the type and accepted by the parser, and Task 13's L1 builder supplies real coverage. Pass `aggregates.resourceCounts.observedAt` from the inventory snapshot's `observedAt` (never the checkpoint write time), and merge stored `resourceCounts` under the caller's so a pass whose caller omits `observedAt`/`resources`/`toolSources` preserves them (controller ruling R35).
