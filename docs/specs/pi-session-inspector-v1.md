...
- **Permission System bus.** `permissions:ready`, `permissions:ui_prompt`, and `permissions:decision` on Pi's public event bus. The broadcasts are best-effort and unversioned: Inspector reads known fields defensively by presence, ignores additive unknown fields, and does not gate on a producer `protocolVersion`; `schemaVersion` belongs only to Inspector's own telemetry envelope. `ready` may repeat for one node and is presence only, never a session/activity counter. `ui_prompt` means the human is about to be asked and counts only that explicit human-facing prompt, not request creation, waiting, or gate evaluation; policy/automatic resolutions may emit no prompt. `decision` fires after every gate resolution, so decisions and prompts are independent and decisions may exceed prompts; Inspector never fabricates a prompt from a decision. Producer `requestId` remains the supported producer-side correlation key for prompt, decision, and review-log entries, but Inspector deliberately does not expose or use it as a semantic join: `origin`, `value`, `matchedPattern`, `agentName`, `forwarding`, and raw `request` are never read outside the permission adapter. The one permitted exception is the hashed `attribution.request` form `permission-request-<canonicalOpaqueDigest("permission-request", sessionId, rawRequestId)>` (ADR 0016 partial supersession of ADR 0014): it is additive envelope metadata only, never folded, never a join key, and never paired with a decision or any other outcome; the raw request ID and its preimage never leave the adapter call frame, and a missing/empty/oversized/malformed ID yields no attribution. A forwarded parent prompt and child decision therefore do not share a cross-session Inspector join key, and a forwarded request answered by the parent's policy may emit no prompt.
