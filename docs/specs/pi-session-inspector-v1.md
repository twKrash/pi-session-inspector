...
`shell.html`, `style.css`, and the generated `client.bundle.js`. The bundle is
produced at build time from the authored sources under `scripts/web/` (route,
range, client) by `npm run build:web`; the server serves the generated bytes as
`/client.js` without runtime module loading. All copy comes from one local
English catalog resolved by an Inspector-owned synchronous translator; there is
no language detector, backend, persistence, or network-loaded resource. The static snapshot does not include
the interactive client. Browsers may probe exact `/favicon.ico`; `GET` and `HEAD`
