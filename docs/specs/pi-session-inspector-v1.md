...
`sessions/<sessionId>/inventory.json` is an analyzer-owned, `schemaVersion: 1`, mode `0o600`, ≤64 KiB sanitized snapshot of the command/skill/resource-source inventory (names, labels, scope/origin, optional bounded descriptions — no paths, bodies, or invocation data). It is atomically renamed into place. Missing or unreadable → inventory `unavailable`, never an empty list. Refresh triggers are session start after tracking promotion and a `resources_discover` event with `reason === "reload"`; each refresh hash-compares the bounded sanitized set and rewrites only on change. Duplicate or unchanged observation writes nothing. Dynamic runtime registrations are therefore picked up at the next reload (or a new session); Inspector never polls or watches the filesystem, and report loads never call the producer (the current report uses the observed in-memory snapshot, history/global read the persisted one).
