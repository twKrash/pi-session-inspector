...
module.exports = {
  forbidden: [
    ...
    // ui/load-history.ts are the approved source-reading loaders.
    {
      ...
      severity: 'error',
      comment:
        'Core owns semantic models and projections; UI is a consumer and must not be imported back into core.',
      from: {
        path: '^src/core/'
      },
      to: {
    ...
    },
    {
      ...
      severity: 'error',
      comment:
        'Core is pure; filesystem, path, and child-process I/O belongs to adapters or the composition root. Core crypto is intentionally allowed for opaque-ID hashing.',
      from: {
        path: '^src/core/'
      },
      to: {
    ...
    },
    {
      ...
      severity: 'error',
      comment:
        'Pi, storage, and integration adapters feed core or are composed by index; they must not depend on renderer/input modules.',
      from: {
        path: '^src/(?:integrations|pi|storage)/'
      },
      to: {
        path: '^src/ui/'
      }
    ...
    {
      name: 'renderers-not-to-raw-sources',
      severity: 'error',
      comment:
        'Renderers and projections consume DTOs, not Pi/storage sources. load-current.ts and load-history.ts are the bounded documented loader exceptions.',
      from: {
        path: '^src/ui/',
        pathNot: '^src/ui/load-(?:current|history)[.]ts$'
      },
      to: {
        path: '^src/(?:pi|storage)/',
        dependencyTypesNot: [
    ...
    {
      name: 'storage-not-to-pi-marker-writer',
      severity: 'error',
      comment:
        'Storage owns Inspector persistence; only the Pi composition adapter may call the tracking marker writer.',
      from: {
        path: '^src/storage/'
      },
      to: {
        path: '^src/pi/tracking-pi[.]ts$'
      }
    },
    {
      ...
      severity: 'error',
      comment:
        'Current L2 loader projects the canonical session; it must not replay the entry reducer a second time.',
      from: {
        path: '^src/ui/load-current[.]ts$'
      },
      to: {
...
// generated: dependency-cruiser@18.2.0 on 2026-09-13T15:16:32.981Z
