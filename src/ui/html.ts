...
function rangeTruncated(){const range=activeRange();if(!range)return false;if(activeSection()==="current"){const current=currentView();return !!current&&current.dailyTruncated===true&&reachesBeforeRetained(current.daily,range);}if(activeSection()==="global")return (data.global.dailyTruncated===true&&reachesBeforeRetained(data.global.daily,range))||partialContribution(range);const session=selectedSession();if(session)return session.availability==="available"&&!!(session.usageByDate||[]).length&&historyRowRange(session,range).partial===true;return partialContribution(range);}
...
// fold's own window and any contributing session's retained window.
