...
const activeEnvTab=()=>activeSettings().envTab||"commands";
const activeMetric=()=>activeSettings().metric||activeMetrics()[0]||"sessions";
const activeToolFilter=()=>toolFilters[viewIdentity()]||null;
const activeIntent=()=>state.range!==undefined?state.range:(rangeIntents[viewIdentity()]||undefined);
...
function sameSessionAgentIds(view){const ids=new Set(),add=candidate=>{const projection=candidate&&candidate.availability==="available"?candidate.report:null;if(!projection||projection.sessionId!==view.sessionId)return;(projection.agents||[]).forEach(run=>{ids.add(run.id)});};if(activeSection()==="current"){add(data.current.active);add(data.current.tree);}else{add(selectedSession()&&selectedSession().view);}(view.agents||[]).forEach(run=>{ids.add(run.id)});return ids;}
...
function openButton(sessionId){const link=el("a","",tr("table.open"));link.dataset.session=sessionId;link.setAttribute("aria-label",tr("table.open")+" "+sessionId);return link;}
...
function backBar(){const bar=el("div","toolbar"),link=el("a","",tr("nav.back"));link.dataset.back="true";bar.append(link);return bar;}
...
function syncTabs(derived){tabsNode.replaceChildren.apply(tabsNode,derived.visibleTabs.map(tab=>tabLink(tab,tab===derived.activeTab)));}
...
// Every id a hash may name, taken from the payload's own bounded id spaces:
// session ids, tool/agent/error row ids, model identities and inventory names. A
// sanitized source label is deliberately absent — a label can be a path, and the
// hash never echoes one (design §9.1).
function knownViews(){const views=[data.current.active,data.current.tree];historySessions().forEach(entry=>{if(entry&&entry.view)views.push(entry.view);});return views;}
// The payload never changes, so the id set is built once: every later navigation
// validates against the same bounded ids.
let knownIdCache=null;
function knownIds(){if(knownIdCache!==null)return knownIdCache;const ids=new Set(),add=value=>{if(typeof value==="string"&&value!=="")ids.add(value);};historySessions().forEach(entry=>add(entry&&entry.sessionId));knownViews().forEach(current=>{[].concat(current.tools||[],current.agents||[],current.errors||[]).forEach(row=>add(row&&row.id));(current.models||[]).forEach(row=>add(row.provider+"/"+row.model));(current.integrations||[]).forEach(row=>add(row.integration));if(current.commands)current.commands.items.forEach(row=>add(row.name));if(current.skills)current.skills.items.forEach(row=>add(row.name));});knownIdCache=ids;return ids;}
...
// the address bar and applies it. A structural navigation pushes a history entry;
// a range or table change replaces the current one, so typing in a search box
// never fills the Back stack.
...
// remembered range decides (§5.3). A null clears a field explicitly.
function routeFor(patch){const section=patch.section!==undefined?patch.section:state.section,rawSession=patch.session!==undefined?patch.session:(section===state.section?state.session:undefined),session=section==="history"&&typeof rawSession==="string"?rawSession:undefined,tab=patch.tab!==undefined?patch.tab:state.tab,next={section:section,tab:tab,scope:patch.scope!==undefined?patch.scope:state.scope};if(session!==undefined)next.session=session;const sameIdentity=viewIdentity(next)===viewIdentity(state),range=patch.range!==undefined?patch.range:(sameIdentity?state.range:undefined);if(range!==undefined)next.range=range;const entity=patch.entity!==undefined?patch.entity:(sameIdentity?state.entity:undefined);if(entity!==undefined)next.entity=entity;const table=patch.table!==undefined?patch.table:(sameIdentity&&tab===state.tab?state.table:undefined);if(table!==undefined)next.table=table;return next;}
...
function adopt(route){state.section=route.section;state.tab=route.tab;state.scope=route.scope;["session","range","entity","table"].forEach(key=>{if(route[key]===undefined)delete state[key];else state[key]=route[key];});}
...
function applyLocation(){const previous={section:state.section,tab:state.tab,session:state.session,entity:state.entity},parsed=parseRoute(location.hash||"",{scope:data.initialScope,capabilities:parseCapabilities(),knownIds:knownIds()}),restored=restoreMemory(parsed.route,previous),derived=deriveView(restored,stateCapabilities(restored),routeDaily(restored).map(row=>row.date)),applied={...restored,tab:derived.activeTab},key=routeKey(applied);if(key===lastAppliedKey)return;lastAppliedKey=key;stateNotice=derived.notice!==undefined?derived.notice:parsed.notice;adopt(applied);render({structural:previous.section!==applied.section||previous.tab!==applied.tab||previous.session!==applied.session||previous.entity!==applied.entity,sectionChanged:previous.section!==applied.section});}
...
document.addEventListener("change",event=>{if(event.target.id==="sort")navigate(withTable({sort:event.target.value==="default"?"":event.target.value}),false);else if(event.target.id==="chart-metric"){setViewSetting("metric",event.target.value);const select=q("chart-metric");if(select&&typeof select.focus==="function")select.focus();}});
...
q("date-form").addEventListener("submit",event=>{event.preventDefault();const from=q("date-from").value,to=q("date-to").value;if(!from||!to||from>to){q("date-error").textContent=tr("range.error");return;}q("date-dialog").close();navigate(routeFor({range:{kind:"custom",from:from,to:to}}),false);});
