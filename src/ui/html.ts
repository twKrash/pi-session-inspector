...
function commandsPanel(view,title){const commands=view.commands;if(!commands||commands.items.length===0)return emptyCard(title,commands&&commands.count!==null?tr("commands.count",{count:number(commands.count)}):tr("unavailable.commands"),"evidence.unavailable");return table(title,tr("commands.note"),[tr("table.name"),tr("table.source"),tr("table.scope"),tr("table.origin"),tr("table.description")],commands.items.map(row=>[linkRow("command",row.name,row.name),orUnavailable(row.sourceLabel||row.source||null),row.scope,row.origin,orUnavailable(row.description)]));}
...
function resourcesCard(resources){if(!resources||resources.state!=="supported"||resources.items.length===0)return unavailableSection(tr("panel.resources"),tr("resources.unavailable"));return simpleTable(card(tr("panel.resources"),tr("resources.note")),[tr("table.source"),tr("table.scope"),tr("table.origin"),tr("table.commands"),tr("table.skills"),tr("table.prompts"),tr("table.tools")],resources.items.map(row=>[linkRow("resource",row.sourceLabel,row.sourceLabel),row.scope,row.origin,number(row.commands),number(row.skills),number(row.prompts),number(row.tools)]));}
...
if(data.toolFilter!==undefined){toolFilters[viewIdentity()]=data.toolFilter;render();return;}if(data.clearFilter!==undefined){toolFilters[viewIdentity()]=null;render();return;}if(data.envTab!==undefined){const named=entityEnvTab();if(named!==undefined&&named!==data.envTab)navigate(routeFor({entity:null}));setViewSetting("envTab",data.envTab);return;}// An entity link (design §9.4) rebuilds exactly the destination its href names,
