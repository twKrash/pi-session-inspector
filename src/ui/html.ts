...
// The report tables and totals are owned by `report-projection.ts`; this module
// only renders them. The block below re-exports the names this module's
// existing consumers still read, so the extract leaves one implementation of
// every transform (this module and its adapter are deleted in the cleanup
// task).
import {
...
// a SessionReportView carries no dated rows. Tools, agents and errors filter their own
...
// Every section's notice reads this one verdict (design §5.1); the global
// aggregate's verdict also reads the contributing sessions' own windows, which
// the same document carries on the history projection (design §5.6).
...
// The history aggregate's one verdict (design §5.6): the fold's own cap plus the
// contributing sessions' windows, so its headline and notice never disagree.
function historyRangeTruncated(range){return (data.history.dailyTruncated===true&&reachesBeforeRetained(data.history.daily,range))||partialContribution(range);}
function rangeTruncated(){const range=activeRange();if(!range)return false;if(activeSection()==="current"){const current=currentView();return !!current&&current.dailyTruncated===true&&reachesBeforeRetained(current.daily,range);}if(activeSection()==="global")return (data.global.dailyTruncated===true&&reachesBeforeRetained(data.global.daily,range))||partialContribution(range);const session=selectedSession();if(session)return session.availability==="available"&&historyRowRange(session,range).partial===true;return historyRangeTruncated(range);}
...
// The window's own truncated flag plus the range-vs-oldest comparison decide it
// (historyRowRange), so an empty, wholly unpublishable window still qualifies.
function partialContribution(range){return historySessions().some(entry=>!!entry&&entry.availability==="available"&&historyRowRange(entry,range).partial===true);}
...
// A member row publishes the verdict's own sum, but only when the session's
// report HAS a published usage summary: an overflowed aggregate dates only
// activity, so its zero-sum window reads Unavailable, never a fabricated zero
// (unavailable != 0), whatever the range's partial verdict says.
function historyRowCells(item,group){const entry=item.entry,verdict=item.verdict,partial=group==="member"&&verdict.partial,member=group==="member",published=member&&!!entry.view&&entry.view.usage!==undefined;return [idValue(entry.sessionId,sessionCell(entry)),orUnavailable(entry.durationLabel),published?(partial?knownValue(number(verdict.totalTokens),"metric.knownTokens"):number(verdict.totalTokens)):tr("evidence.unavailable"),entry.generationCount===null?tr("evidence.unavailable"):number(entry.generationCount),entry.agentCount===null?tr("evidence.unavailable"):number(entry.agentCount),entry.status?badge(tr(entry.status.key),entry.status.tone):tr("evidence.unavailable"),published?(partial?knownValue(money(verdict.cost),"metric.knownCost"):money(verdict.cost)):tr("evidence.unavailable"),entry.view?openButton(entry.sessionId):""];}
...
// §5.6), exactly as the history aggregate's does: the verdict reads both the
// fold's own window and any contributing session's retained window. An aggregate
// with no datable row has no range and no figure to state: it reads Unavailable,
// never a fabricated zero (invariant: unavailable != 0).
function globalOverview(){const range=activeRange(),days=selectedDays(),empty=!!range&&days.length===0,datable=!!range&&!!(data.global.daily||[]).length,tokens=days.reduce((sum,row)=>sum+row.totalTokens,0),cost=days.reduce((sum,row)=>sum+row.cost,0),labels=data.global.usageLabels,partial=rangeTruncated(),costLabel=partial&&labels.cost==="metric.cost"?"metric.knownCost":labels.cost,tokensLabel=partial&&labels.tokens==="metric.tokens"?"metric.knownTokens":labels.tokens,costValue=labels.usageUnavailable?tr("metric.costUnavailable"):(datable?money(cost):tr("evidence.unavailable")),tokensValue=labels.usageUnavailable?tr("metric.costUnavailable"):(datable?number(tokens):tr("evidence.unavailable")),metrics=empty?rangeEmpty():el("div","metrics");if(!empty)metrics.append(metric(tr(costLabel),costValue,tr("metric.native"),[[tr("table.date"),rangeText()]]),metric(tr(tokensLabel),tokensValue,tr("metric.tokens.note"),[[tr("table.date"),rangeText()]]),metric(tr("metric.days"),datable?number(days.length):tr("evidence.unavailable"),tr("metric.days.note"),[[tr("range.label"),rangeText()]]));const all=el("div","");all.append(metrics,coveragePanel(data.global),compositionCard(data.global.composition),evidencePanel(activeEvidence()));return all;}
