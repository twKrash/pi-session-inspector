 ...
 * the machine clock, so exports stay byte-identical for identical reports. The
 * window is shifted by the range module's own UTC day arithmetic, so the legacy
 * adapter and the client cannot disagree about what `14D` means.
 */
export function defaultReportPeriod(
  ...
  // `shiftUtcDay` refuses a malformed date; the latest observed date is the
  // only honest anchor left, and the daily rows always carry a valid date.
  return { preset: 14, from: shiftUtcDay(to, -13) ?? to, to };
}
...
// The one range filter every range-aware widget reads (design §5.2/§5.4). Row
// sets without a usable date are not passed in: tools and agents keep their
// projected rows and say All report dates until their time fields land.
function filteredView(view){const range=activeRange();if(!range)return null;return filterView({rows:[],models:(view&&view.datedModels)||[],tools:[],agents:[],errors:(view&&view.errors)||[]},range);}
...
function rangeTruncated(){const range=activeRange();if(!range)return false;if(state.section==="current")return !!(currentView()&&currentView().dailyTruncated);if(state.section==="global")return data.global.dailyTruncated===true;const session=selectedSession();if(session)return session.availability==="available"&&!!(session.usageByDate||[]).length&&historyRowRange(session,range).partial===true;return partialContribution(range);}
function syncRangeNotice(){const text=activeIntent()!==undefined&&!activeRange()?tr("range.restored"):(rangeTruncated()?tr(state.section==="current"?"range.truncated":"history.dailyTruncated"):"");let notice=q("range-truncated");if(!text){if(notice)notice.hidden=true;return;}if(!notice){notice=el("p","range-note",text);notice.id="range-truncated";q("time-range").append(notice);}notice.textContent=text;notice.hidden=false;}
...
function knownValue(value,key){const node=el("span","",value+" ");node.append(badge(tr(key),"warn"));return node;}
...
function emptyOverview(){const node=el("div","");node.append(unavailableSection(tr("usage.title"),tr("chart.empty")),evidencePanel(activeEvidence()));return node;}
...
// filter, so a range change genuinely changes model figures (design §5.2/§5.4).
// A payload without a dated projection (the legacy adapter) keeps its aggregate
// rows and states that they are all report dates.
...
function historyOverview(){const range=activeRange(),classified=historyRows(),members=historyMetrics(classified.members),totals=range?periodTotals(activeDaily(),range):null,labels=data.history.usageLabels,costLabel=members.partial&&labels.cost==="metric.cost"?"metric.knownCost":labels.cost,tokensLabel=members.partial&&labels.tokens==="metric.tokens"?"metric.knownTokens":labels.tokens,attributable=members.sessions>0||classified.unknown.length===0,costValue=labels.usageUnavailable?tr("metric.costUnavailable"):(totals&&attributable?money(totals.cost):tr("evidence.unavailable")),tokensValue=labels.usageUnavailable?tr("metric.costUnavailable"):(totals&&attributable?number(totals.totalTokens):tr("evidence.unavailable")),blocks=el("div","metrics");blocks.append(metric(tr(costLabel),costValue,tr("metric.native"),[[tr("table.date"),rangeText()]]),metric(tr(tokensLabel),tokensValue,tr("metric.tokens.note"),[[tr("table.date"),rangeText()]]),metric(tr("metric.generations"),totals?number(totals.generations):tr("evidence.unavailable"),tr("metric.generations.note"),[[tr("table.date"),rangeText()]]),metric(tr("metric.sessions"),number(members.sessions),tr("history.sessions.note"),[[tr("evidence.native"),number(historySessions().length)+" tracked"]]));const all=el("div","");all.append(blocks,coveragePanel(data.history),historyTable(classified),evidencePanel(activeEvidence()));return all;}
...
function globalOverview(){const days=selectedDays(),tokens=days.reduce((sum,row)=>sum+row.totalTokens,0),cost=days.reduce((sum,row)=>sum+row.cost,0),labels=data.global.usageLabels,costValue=labels.usageUnavailable?tr("metric.costUnavailable"):money(cost),tokensValue=labels.usageUnavailable?tr("metric.costUnavailable"):number(tokens),metrics=el("div","metrics");metrics.append(metric(tr(labels.cost),costValue,tr("metric.native"),[[tr("table.date"),rangeText()]]),metric(tr(labels.tokens),tokensValue,tr("metric.tokens.note"),[[tr("table.date"),rangeText()]]),metric(tr("metric.days"),number(days.length),tr("metric.days.note"),[[tr("range.label"),rangeText()]]));const all=el("div","");all.append(metrics,coveragePanel(data.global),compositionCard(data.global.composition),evidencePanel(activeEvidence()));return all;}
