...
const HELP_LINES: readonly string[] = Object.freeze([
  ...
  `  snapshot  ${INSPECTOR_TARGETS.snapshot.join(" | ")} <sessionId>`,
  `  tui       ${INSPECTOR_TARGETS.tui.join(" | ")}`,
  `  json      ${INSPECTOR_TARGETS.json.join(" | ")} <sessionId>`,
  "Options:",
  ...
  "  snapshot: current scope+range; history/global range; session neither",
  `  tui       ${INSPECTOR_OPTIONS.tui.join(" ")}`,
  `  json      ${INSPECTOR_OPTIONS.json.join(" ")}; scope current only, else full tree`,
  ...
  "  /session-inspector json session session-a --output report.json",
]);
